...
- `BACKEND_INVALID: <name> (config/secondmate-backend; ...)` - the same class of problem scoped to `config/secondmate-backend`: the value is not `default` and not a backend with a verified secondmate launch design, so do not spawn or respawn secondmates until it is corrected to one of the listed values.
