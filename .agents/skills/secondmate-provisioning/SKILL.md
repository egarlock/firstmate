...
`config/providers/` provider-env files are deliberately not inherited either, so an inherited `config/crew-dispatch.json` that names a provider needs the matching `config/providers/<name>.env` created by hand in the secondmate home (`bin/fm-config-inherit-lib.sh`; `docs/configuration.md` "Provider environment files"), and a remote route refuses `--provider` outright.
