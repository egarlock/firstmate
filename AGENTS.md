...
3. **Never tear down unlanded work.**
   Uncommitted changes are never landed, and `bin/fm-teardown.sh` owns the complete landed-work test: a dirty gate, then a recorded `landed=` merge verdict that covers HEAD, reachability from a publishing remote, or content already in the default branch.
...
config/crew-dispatch.json  optional crewmate dispatch profiles; LOCAL, gitignored; firstmate-maintained but human-editable natural-language rules that choose a per-task harness/model/effort/provider profile (section 4). Inherited by secondmate homes
config/providers/<name>.env  optional Anthropic-compatible endpoint env files for claude launches, selected per spawn with fm-spawn --provider and referenced by dispatch profiles; LOCAL, gitignored; values stay unexpanded and the grammar is owned by docs/configuration.md "Provider environment files"; deliberately NOT inherited into secondmate homes
config/secondmate-harness  harness the PRIMARY uses to launch SECONDMATE agents, optionally followed by a model and effort token on the same line ("<harness> [<model>] [<effort>]"; section 4); LOCAL, gitignored; absent or "default" harness falls back to config/crew-harness then firstmate's own. The primary's own setting; NOT inherited into secondmate homes (secondmates do not spawn secondmates)
config/secondmate-backend  runtime backend the PRIMARY uses to launch SECONDMATE agents; LOCAL, gitignored; absent or "default" falls through to the config/backend resolution chain, an explicit per-spawn --backend always wins, and a respawn reuses the backend recorded in the mate's own record (docs/configuration.md "Runtime backend"); bootstrap flags invalid values; the primary's own setting, NOT inherited into secondmate homes
...
config/cmux-container  cmux-backend task-container shape; LOCAL, gitignored; absent or "workspace" = one fm-titled workspace per task (the default), "tab" = task tabs join firstmate's own cmux workspace, or a shared per-home workspace outside cmux (docs/cmux-backend.md "Task container shape"); not inherited into secondmate homes
config/cmux-socket-password  optional cmux control-socket password; LOCAL, gitignored; read fresh on every cmux CLI call and passed through without ever overriding an operator's own ambient CMUX_SOCKET_PASSWORD when absent (docs/cmux-backend.md "Setup")
config/wedge-alarm  optional away-mode wedge-alarm active-alert directives; LOCAL, gitignored; absent means auto (macOS Notification Center when available); see docs/wedge-alarm.md
config/harbor-watch.json  optional two-writer harbor-watch list of the captain's own opted-in cmux tabs; LOCAL, gitignored; schema owned by docs/configuration.md "Harbor watch", mechanics by bin/fm-harbor-watch.sh, chat ops and escalation policy by the harbor-watch skill (section 13)
...
state/               volatile runtime signals; gitignored
  ...
  harbor-watch.check.sh harbor-watch.log .harbor-watch-reported  harbor-watch sweep shim (present only while armed), append-only audit log, and escalation dedupe ledger (bin/fm-harbor-watch.sh; section 13 harbor-watch skill)
...
Both record the `landed=` verdict at merge time, which is what teardown reads to confirm the work landed instead of re-deriving it.
...
3. For `check:`, act on the named poll result, including merges, X-mode events, and harbor-watch tab events (section 13's `harbor-watch` skill).
...
- `harbor-watch` - load on any `check:` wake from `state/harbor-watch.check.sh`, before opting one of the captain's own cmux tabs in or out of harbor watch or listing the watches, and before relaying a harbor-watch escalation; wave 1 is observe-only and the skill owns that contract.
