...
3. **Never tear down a worktree that holds unlanded work.**
   ...
   The landed-work oracle is a dirty gate followed by three allow-conditions, checked in order: a dirty worktree (uncommitted changes) is never landed and always refuses; then it allows on any one of a recorded `landed=<sha>` line in `state/<id>.meta` (the merge already happened), OR HEAD reachable from any publishing remote-tracking branch (a fork counts, the local no-mistakes gate remote excluded, so the work is already published), OR the branch's content already present in the up-to-date default branch (after any merge - squash, rebase, ff, or a `local-only` fast-forward - the change is in the default branch regardless of whether the branch's own commits survived); it refuses only when none of the three holds or the content check is inconclusive.
   The `landed=<sha>` verdict is the authoritative "this task's work reached its destination" fact, recorded at the moment of merge by `bin/fm-pr-merge.sh` (the merged PR head sha, or `pr-<n>` when no head is available) and by `bin/fm-merge-local.sh` (the merged default-branch tip); recording it at merge time is what lets teardown stop re-deriving the verdict from the fragile PR-head-ancestor and patch-id-replay heuristics - heuristics that false-refused after a no-mistakes run pushed an advanced head to origin while the local worktree HEAD lagged. The direct remote-reachability allow-condition is retained (it is the base "landed" definition), but the ancestor and patch-id matching stay removed.
...
The landed-work oracle is a dirty gate followed by three allow-conditions in order: a dirty worktree (uncommitted changes) always refuses; then it allows on any one of a recorded `landed=<sha>` in `state/<id>.meta` (the merge already happened, recorded by `bin/fm-pr-merge.sh` / `bin/fm-merge-local.sh`), OR HEAD reachable from any publishing remote-tracking branch (a fork counts, the local no-mistakes gate remote excluded), OR the branch's content already present in the up-to-date default branch; it refuses only when none holds.
The `landed=` verdict is what makes teardown robust to a no-mistakes run that advanced origin past the local worktree HEAD: the merge was recorded when it happened, so teardown no longer re-derives it from the fragile PR-head-ancestor or patch-id-replay heuristics (the direct remote-reachability allow-condition is retained; only the ancestor and patch-id matching were dropped).
