...
For a `--secondmate` spawn, `bin/fm-spawn.sh` populates `MODEL`/`EFFORT` from those tokens only when the caller passed no explicit `--model`/`--effort`; an explicit per-spawn flag always wins over the file's tokens.
