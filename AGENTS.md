...
3. **Never tear down a worktree that holds unlanded work.**
   ...
   The landed-work oracle is a recorded verdict plus one fallback, checked in order: a dirty worktree (uncommitted changes) is never landed and always refuses; a recorded `landed=<sha>` line in `state/<id>.meta` means the merge already happened and allows; otherwise the single fallback asks whether the branch's content is already present in the up-to-date default branch (after any merge - squash, rebase, ff, or a `local-only` fast-forward - the change is in the default branch regardless of whether the branch's own commits survived), allowing when it is and refusing when it is not or is inconclusive.
   The `landed=<sha>` verdict is the authoritative "this task's work reached its destination" fact, recorded at the moment of merge by `bin/fm-pr-merge.sh` (the merged PR head sha, or `pr-<n>` when no head is available) and by `bin/fm-merge-local.sh` (the merged default-branch tip); recording it at merge time is what lets teardown stop re-deriving the verdict from remote-reachability, PR-head-ancestor, and patch-id heuristics - heuristics that false-refused after a no-mistakes run pushed an advanced head to origin while the local worktree HEAD lagged.
...
The landed-work oracle is three checks in order: a dirty worktree (uncommitted changes) always refuses; a recorded `landed=<sha>` in `state/<id>.meta` allows (the merge already happened, recorded by `bin/fm-pr-merge.sh` / `bin/fm-merge-local.sh`); otherwise the single fallback asks whether the branch's content is already present in the up-to-date default branch and allows when it is.
The `landed=` verdict is what makes teardown robust to a no-mistakes run that advanced origin past the local worktree HEAD: the merge was recorded when it happened, so teardown no longer re-derives it from remote-reachability, PR-head-ancestor, or patch-id heuristics.
The content fallback fetches the default branch fresh; that fetch is time-bounded (`FM_TEARDOWN_NET_TIMEOUT`, default 30s) so a hung remote cannot foreground-block teardown, and the tree comparison requires git >= 2.38 (for `git merge-tree --write-tree`), printing a clear diagnostic and refusing on older git rather than silently false-refusing.
The fallback also covers the no-remote `local-only` case by comparing against `refs/heads/<default>` when there is no origin.
Genuinely unlanded work (no recorded `landed=` and content not in the default branch) and dirty worktrees still refuse; an inconclusive content check refuses rather than silently allowing.
