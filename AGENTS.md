...
3. **Never tear down a worktree that holds unlanded work.**
   ...
   The landed-work oracle is a dirty gate followed by three allow-conditions, checked in order: a dirty worktree (uncommitted changes) is never landed and always refuses; then it allows on any one of a recorded `landed=<sha>` line in `state/<id>.meta` that covers HEAD (HEAD is an ancestor of the recorded sha - the merge already happened and nothing was committed past it), OR HEAD reachable from any publishing remote-tracking branch (a fork counts, the local no-mistakes gate remote excluded, so the work is already published), OR the branch's content already present in the up-to-date default branch (after any merge - squash, rebase, ff, or a `local-only` fast-forward - the change is in the default branch regardless of whether the branch's own commits survived); it refuses only when none of the three holds or the content check is inconclusive.
   The `landed=<sha>` verdict is the authoritative "this task's work reached its destination" fact, recorded at the moment of merge by `bin/fm-pr-merge.sh` (the merged PR head sha, or `pr-<n>` when no head is available) and by `bin/fm-merge-local.sh` (the merged default-branch tip); recording it at merge time is what lets teardown stop re-deriving the verdict from the fragile PR-head-ancestor and patch-id-replay heuristics - heuristics that false-refused after a no-mistakes run pushed an advanced head to origin while the local worktree HEAD lagged. The direct remote-reachability allow-condition is retained (it is the base "landed" definition), but the ancestor and patch-id matching stay removed. The verdict vouches only for the commits it covers: a `landed=` that no longer covers HEAD (commits made after the merge) or that cannot be resolved (the `pr-<n>` placeholder) falls through to the other two conditions instead of allowing, so post-merge commits are never silently discarded.
   Use `bin/fm-pr-merge.sh <id> <full GitHub or Azure DevOps PR URL>` for every merge (captain-requested or yolo) so `pr=`, any available `pr_head=`, and `landed=` are recorded as part of the merge itself.
...
config/cmux-container  cmux-backend task-container shape; LOCAL, gitignored; absent or "tab" = task tabs join firstmate's own cmux workspace (or a shared "firstmate" workspace outside cmux), "workspace" = one fm-<id> workspace per task (docs/cmux-backend.md); not inherited into secondmate homes
...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=; kind=secondmate also records home= and projects=; a task on a non-default runtime backend also records backend= (absent means tmux, the verified reference backend; herdr is an experimental backend recording herdr_session=, herdr_workspace_id=, herdr_tab_id=, herdr_pane_id= too - docs/herdr-backend.md; cmux is an experimental backend recording cmux_workspace_id=, plus cmux_surface_id= for a tab-mode task, too - docs/cmux-backend.md; bin/fm-backend.sh, section 8) (fm-pr-check, including through fm-pr-merge, appends pr= and GitHub's pr_head= when available; fm-pr-merge and fm-merge-local append landed=<sha> on a successful merge as teardown's authoritative landed verdict; fm-x-link appends x_request= and x_request_ts= for an X-mention-originated task, section 14)
...
For the cmux backend, the task endpoint is an `fm-<id>` tab in firstmate's own workspace by default (target `cmux:<workspace-uuid>:<surface-uuid>`), or an `fm-<id>` workspace per task when `config/cmux-container` is `workspace` (target `cmux:<workspace-uuid>`).
...
The executable allowlist and per-adapter model/effort capability live in `bin/fm-harness-policy.sh`, the single source `fm-spawn.sh`, `fm-bootstrap.sh`, `fm-harness.sh`, and `fm-lock.sh` all read; the lists in this section are the human mirror.
...
1. Run `bin/fm-lock.sh` to acquire the session lock (`state/.lock`).
   Acquisition is atomic and identity-verified: it claims the lock through the same portable `fm_lock_try_acquire` primitive the watcher/queue locks use (a symlink-to-owner-dir create that is atomic on POSIX and needs no `flock`), so two sessions starting at once resolve to exactly one winner, and it records the session-stable harness PID plus that process's `pid-identity` (start time + command, via `fm_pid_identity`). A contended holder is trusted only when it is genuinely alive AND its recorded identity still matches (guarding against a reused/recycled PID) AND it is a firstmate harness; a holder that fails any of those is stale and is reclaimed atomically. Recorded identities are format-tagged, because one firstmate version writes them and another reads them back, and because the same live PID can be described by either identity source (Linux `/proc` or portable `ps`, which the reader falls back to whenever a `/proc` read does not parse): an identity recorded in a different format is unverifiable rather than mismatched, so a live harness holding it is refused (and its identity healed in place) instead of being mistaken for a recycled PID and having its lock stolen. Refusing must not become refusing forever, so the unverifiable branch still requires a live harness, and an unreaped (defunct) process is not one - it stays reclaimable rather than leaving the home read-only until someone reaps it. A leftover pre-directory plain-file `.lock` is migrated on the next acquire.
...
For ship and scout tasks, the script creates the runtime endpoint (a tmux window by default, a herdr tab/pane when `backend=herdr`, or a cmux tab in firstmate's workspace - or its own workspace, per `config/cmux-container` - when `backend=cmux`), runs `treehouse get`, waits for the worktree subshell, asserts the resolved worktree is a genuine isolated worktree distinct from the primary checkout (aborting the spawn otherwise, to prevent the worktree tangle of section 8), installs the turn-end hook, records `state/<id>.meta`, and launches the agent with the brief.
...
A grok or copilot task that dies without teardown leaves its per-task registry token orphaned under `hooks/fm-turn-end.d/`; `bin/fm-hook-sweep.sh` (run best-effort by bootstrap, section 3) removes such orphans - home-agnostically and age-guarded against in-flight spawns - so the shared hook registry never accumulates cruft, and it never touches a live token or another home's token.
...
It also fixes the other half of that staleness: once `pr=` is recorded, the *compare* side is resolved from the forge rather than taken from the local branch, because a no-mistakes fix round pushes to the open PR and leaves the local worktree behind - approving that diff would approve a change missing fixes that already landed, unattended under `yolo=on`.
Each forge is asked for the PR's head afresh on every run: GitHub re-fetches `refs/pull/<n>/head`, and Azure DevOps - which publishes no such ref - re-queries `lastMergeSourceCommit` through the az CLI.
An Azure DevOps head that is not already local is fetched by object name, and failing that through the PR's source branch ref, because Azure Repos only serves a commit that is an advertised ref tip.
The recorded `pr_head=` is a one-shot snapshot `bin/fm-pr-check.sh` took when the PR was first recorded and nothing refreshes it, so it is only the fallback for when the live lookup cannot run (no network on GitHub; no az CLI or a failed query on Azure DevOps).
A live Azure DevOps head that is known but cannot be fetched falls back to that snapshot with its own stderr warning, because firstmate then knows it is reviewing something other than the PR's head.
If nothing resolves, the helper diffs the local branch and says so on stderr - a warning to take seriously, because the review may lag the PR.
Every run prints the resolved compare side next to the base, so an approval records exactly what was reviewed.
...
`bin/fm-pr-merge.sh` always records `pr=` and records `pr_head=` when available before merging, then records `landed=<sha>` once the merge itself succeeds (teardown's authoritative landed verdict), parses the full `https://github.com/<owner>/<repo>/pull/<n>` URL into `gh-axi pr merge <n> --repo <owner>/<repo>`, and defaults to `--squash` unless an explicit merge method is forwarded after `--`; this holds even on a repo with no PR CI where the "checks green" signal that normally triggers `bin/fm-pr-check.sh` never fires - do not call `gh-axi pr merge` directly for a task's PR, or the recording step can be silently skipped and a later `fm-teardown.sh` has no recorded `landed=` verdict and must fall back to the content check.
...
If the captain says "merge it", run `bin/fm-pr-merge.sh <id> <full GitHub or Azure DevOps PR URL>` yourself; that instruction is the explicit approval.
...
The landed-work oracle is a dirty gate followed by three allow-conditions in order: a dirty worktree (uncommitted changes) always refuses; then it allows on any one of a recorded `landed=<sha>` in `state/<id>.meta` that covers HEAD (HEAD is an ancestor of the recorded sha; the merge already happened, recorded by `bin/fm-pr-merge.sh` / `bin/fm-merge-local.sh`), OR HEAD reachable from any publishing remote-tracking branch (a fork counts, the local no-mistakes gate remote excluded), OR the branch's content already present in the up-to-date default branch; it refuses only when none holds.
The `landed=` verdict is what makes teardown robust to a no-mistakes run that advanced origin past the local worktree HEAD: the merge was recorded when it happened, so teardown no longer re-derives it from the fragile PR-head-ancestor or patch-id-replay heuristics (the direct remote-reachability allow-condition is retained; only the ancestor and patch-id matching were dropped), and a lagging local HEAD is still covered because it is an ancestor of the recorded merged head.
The verdict vouches only for the commits it covers: commits made after the merge de-cover HEAD, and an uncovered or unresolvable (`pr-<n>`) `landed=` falls through to the reachability and content conditions instead of allowing, so post-merge work is never silently discarded.
A worktree git cannot read (a broken `.git` pointer, a corrupted repo, a non-git directory) refuses up front rather than letting empty git output read as clean and published.
The reachability check is cheap and local, so it runs before the content fallback; the content fallback fetches the default branch fresh; that fetch is time-bounded (`FM_TEARDOWN_NET_TIMEOUT`, default 30s) so a hung remote cannot foreground-block teardown, and the tree comparison requires git >= 2.38 (for `git merge-tree --write-tree`), printing a clear diagnostic and refusing on older git rather than silently false-refusing.
The fallback also covers the no-remote `local-only` case by comparing against `refs/heads/<default>` when there is no origin.
...
If teardown false-refuses because the merge was performed by hand rather than through `bin/fm-pr-merge.sh` (so no `landed=` was recorded) and the content is not yet visible in the local default ref, fetch the default branch (or re-run the merge through `bin/fm-pr-merge.sh`) and retry - never reach for `--force`.
