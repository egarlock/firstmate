...
3. **Never tear down a worktree that holds unlanded work.**
   ...
   Use `bin/fm-pr-merge.sh <id> <full GitHub PR URL>` for every merge (captain-requested or yolo) so `pr=`, any available `pr_head=`, and `landed=` are recorded as part of the merge itself.
