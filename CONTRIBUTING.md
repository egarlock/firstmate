...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, and `.agents/skills/`.
  ...
  A local `config/backend` file explicitly overrides runtime auto-detection for new task endpoints and stays gitignored; accepted values are `tmux` and the experimental `herdr` and `cmux`.
...
tests/fm-pr-url-lib.test.sh               # shared PR-URL classifier (fm-pr-url-lib.sh): GitHub and both Azure DevOps hosts classified with fields extracted and the org URL normalized, unknown forges / injection / garbage refused
tests/fm-pr-check.test.sh                 # fm-pr-check.sh PR-URL validation: well-formed GitHub and Azure DevOps URLs accepted and check shims armed (ADO poll silent on active, waking on completed/abandoned), command-substitution URL rejected before any write, unknown-forge URL rejected, missing azure-devops extension refused with the remedy
tests/fm-pr-merge.test.sh                 # fm-pr-merge.sh records pr= and available pr_head= before merging, parses PR URLs into gh-axi number/--repo calls, defaults to squash, preserves explicit merge methods, rejects malformed URLs and repo overrides, propagates real merge failures, and for Azure DevOps URLs completes via az repos pr update, records landed= from lastMergeCommit (pr-<n> fallback), refuses -- merge args, and refuses without the azure-devops extension
...
tests/fm-promote.test.sh                  # fm-promote.sh scout->ship promotion: the kind= flip preserves every other meta field, while a non-scout task and a missing meta refuse and leave meta untouched
...
tests/fm-dashboard.test.sh                # fm-dashboard.sh read-only fleet view: empty fleet idle state, busy-fleet rows + backlog render, needs-attention band (working excluded), mode-aware PR pill + clickable link, harness/kind/mode/yolo chips, HTML escaping, strict read-only over state//data/, and --stdout writing no file and not invoking lavish
