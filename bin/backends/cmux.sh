...
# Harness command-name recognition for the recovery-grade agent-state
# classifier below comes from the shared session-lock identity lib
# (fm_harness_path_name), which itself reads the single policy source
# (bin/fm-harness-policy.sh) - the same import bin/backends/tmux.sh uses, so a
# newly verified adapter registers once and both classifiers pick it up.
# shellcheck source=bin/fm-session-lock-lib.sh
. "$FM_BACKEND_CMUX_ROOT/bin/fm-session-lock-lib.sh"
...
# fm_backend_cmux_classify_process_name: the process-name vocabulary for the
# recovery-grade classifier below - `agent` for a verified harness, `shell`
# for an idle shell, `other` for anything else. The near-twin of
# bin/backends/tmux.sh's fm_backend_tmux_classify_process_name (same fast
# path, same fm_harness_path_name fallback so version-named installs such as
# Claude Code's ~/.local/share/claude/versions/<v> still identify by path
# component), with ONE deliberate cmux-specific member: `login`, because cmux
# parents every tab shell through a resident /usr/bin/login wrapper (verified
# live; docs/verification/runtime-backends.md "cmux"), which must read as part
# of an idle shell stack, never as an unattributable process.
fm_backend_cmux_classify_process_name() {  # <path> [argv0] -> agent|shell|other
  local path=$1 argv0=${2:-} base
  base=${path##*/}
  base=${base#-}
  case "$base" in
    *claude*|*codex*|*opencode*|*grok*|*kimi*|*copilot*|pi|pi-signed|pi-launcher|Pi) printf 'agent' ;;
    zsh|bash|sh|dash|ash|ksh|mksh|tcsh|csh|fish|login) printf 'shell' ;;
    *)
...
# shared six-state vocabulary and docs/verification/runtime-backends.md "cmux"
# for the empirical basis. Only `dead` and `missing` license recovery, so
# every rung below errs toward the non-actionable states.
#
# Socket tiers first (nothing behind an unreachable socket can be inspected):
#   - `down` (no control socket) -> missing: cmux hosts every task pty, so no
#     cmux-backed agent process can exist without the app, and the recorded
#     endpoint is authoritatively absent right now. A respawn relaunches the
#     app through fm_backend_cmux_ensure_running, whose create-time duplicate
#     checks refuse loudly if a session-restored workspace conflicts.
#   - denied/unauth/error -> unreadable: an auth or transport failure proves
#     nothing about the agent and must never license a duplicate.
#
# Identity next. A SECONDMATE expected-label delegates to the id-primary
# resolver (fm_backend_cmux_secondmate_resolve - recorded ids, synced recorded
# title, scoped tab title, home-cwd fingerprint), exactly like
# fm_backend_cmux_target_ready's routine-ops delegation, but keeping the
# resolver's outcome distinctions the readiness funnel collapses:
#   resolved -> probe the resolved endpoint; gone (rc 2) -> missing;
#   multiple candidates (rc 3) -> ambiguous; uninspectable -> unreadable.
# An ordinary target re-resolves through target_ready; when that fails, a
# still-readable workspace inventory proves the endpoint absent (missing),
# while an unreadable inventory stays unreadable.
#
# Process attribution last, on the resolved surface's tty: classify EVERY
# process on the tty (never just the last foreground entry - a live agent's
# tool child, e.g. a bash it shelled out to, can transiently be the foreground
# process and must not read as a dead shell):
#   any verified-harness name -> alive;
#   nothing but shells (including cmux's resident login wrapper) -> dead;
#   anything else (pi's bare "node", an editor) -> ambiguous.
# A fresh surface whose lazily-started terminal never started has no tty and
# no processes to attribute -> ambiguous; an empty ps read -> unreadable.
fm_backend_cmux_agent_state() {  # <target> [expected-label]
  local target=$1 expected_label=${2:-} state meta pair rc tty comms line only_shells=1 any=0
  state=$(fm_backend_cmux_ping_state)
  case "$state" in
    ok) : ;;
  ...
  meta=""
  [ -z "$expected_label" ] || meta=$(fm_backend_cmux_secondmate_meta_for_label "$expected_label")
  if [ -n "$meta" ]; then
    rc=0
    pair=$(fm_backend_cmux_secondmate_resolve "$meta" 2>/dev/null) || rc=$?
    case "$rc" in
      0)
        FM_BACKEND_CMUX_WORKSPACE=${pair%%:*}
        FM_BACKEND_CMUX_SURFACE=${pair#*:}
      ...
      3) printf 'ambiguous'; return 0 ;;
      *) printf 'unreadable'; return 0 ;;
    esac
  elif ! fm_backend_cmux_target_ready "$target" "$expected_label"; then
  ...
  while IFS= read -r line; do
    [ -n "$line" ] || continue
    any=1
    case "$(fm_backend_cmux_classify_process_name "$line")" in
...
# agent verdict. The detailed state contract is owned by
# fm_backend_cmux_agent_state, mirroring bin/backends/tmux.sh's pair.
fm_backend_cmux_agent_alive() {  # <target> [expected-label]
  case "$(fm_backend_cmux_agent_state "$1" "${2:-}")" in
    alive) printf 'alive' ;;
