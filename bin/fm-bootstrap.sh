...
# The version probe and compare come from the shared policy lib (sourced near
# the top of this script): fm_harness_version_parts anchors to the FIRST dotted
# triple on the version line and fm_version_ge does the field-wise integer
# compare, so the parser cannot drift between this gate, the copilot spawn gate,
# and the tasks-axi probe. The probe this replaced took the LAST triple, so a
# trailing build or runtime stamp won the match and an incompatible
# `no-mistakes version v1.30.0 (go1.21.5)` parsed as 1.21.5 and passed the gate.
