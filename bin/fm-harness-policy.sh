...
#   bin/fm-session-lock-lib.sh harness process recognition (fm_harness_process_re,
#                              fm_harness_path_names)
#   bin/fm-supervision-instructions.sh  protocol snippet allowlist
#   bin/fm-remote-secondmate-control.sh remote secondmate harness allowlist
#
...
# detect_own, the busy signature in bin/fm-busy-lib.sh (semantic sources or a
# verified rendered fallback) and bin/fm-tmux-lib.sh (delivery-guard footer),
# and the knowledge section in the harness-adapters skill.
...
#             pi-signed is the same engine behind the signed wrapper.
...
#   kimi      no reasoning-effort flag; a requested effort stays in task metadata
#             but never reaches the launch command.
...
FM_VERIFIED_ADAPTERS='claude codex opencode pi pi-signed grok kimi copilot'
...
fm_harness_efforts() {
  case "$1" in
    claude|copilot|pi|pi-signed) printf '%s\n' 'low medium high xhigh max' ;;
...
# harness by process command name (fm-session-lock-lib.sh's holder liveness).
# pi and pi-signed are anchored because a two-letter substring false-positives
# everywhere and the signed wrapper should match only as its exact name; every
# other adapter name is distinctive enough for substring matching.
fm_harness_process_re() {
  local a out=
  for a in $FM_VERIFIED_ADAPTERS; do
    case "$a" in
      pi) a='^pi$' ;;
      pi-signed) a='^pi-signed$' ;;
    esac
    out="${out:+$out|}$a"
  done
  printf '%s\n' "$out"
...
# fm_harness_path_names: the verified adapters as exact executable names, one
# per line, for whole-path-component matching (fm-session-lock-lib.sh's
# fm_harness_path_name). Exact-component matching makes ordering irrelevant.
fm_harness_path_names() {
  local a
  for a in $FM_VERIFIED_ADAPTERS; do printf '%s\n' "$a"; done
}

# fm_harness_from_comm <basename>: map a process command basename to the
# verified adapter it belongs to (substring match; pi, pi-signed, and kimi
# exact), or fail. A pi-signed command reports pi: the signed wrapper is the
# same engine, and the pi-signed IDENTITY is asserted only by the explicit
# FM_PI_HARNESS launch marker (bin/fm-harness.sh detect_own), never inferred
# from ancestry.
fm_harness_from_comm() {
  ...
  for h in $FM_VERIFIED_ADAPTERS; do
    case "$h" in
      ...
      pi-signed) [ "$1" = pi-signed ] && { echo pi; return 0; } ;;
      kimi) [ "$1" = kimi ] && { echo kimi; return 0; } ;;
...
# matched only as a standalone word or path tail; pi-signed and kimi only as a
# standalone word or path tail as well, since their names could otherwise
# shadow ordinary path text.
fm_harness_from_args() {
  local h
  for h in $FM_VERIFIED_ADAPTERS; do
    case "$h" in
      pi) case "$1" in *' pi '*|*/pi) echo pi; return 0 ;; esac ;;
      pi-signed) case "$1" in *' pi-signed '*|*/pi-signed) echo pi; return 0 ;; esac ;;
      kimi) case "$1" in *' kimi '*|*/kimi) echo kimi; return 0 ;; esac ;;
      *) case "$1" in *"$h"*) echo "$h"; return 0 ;; esac ;;
    esac
  done
  return 1
