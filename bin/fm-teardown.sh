...
validate_worktree_teardown_safety() {
  ...
  dirty=$(printf '%s\n' "$dirty_raw" | grep -vE '^\?\? (\.claude/|\.fm-(grok|kimi|copilot)-turnend$)' | head -1 || true)
...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      if [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
        ...
        rm -f "$child_wt/.claude/settings.local.json" "$child_wt/.opencode/plugins/fm-turn-end.js" \
          "$child_wt/.fm-grok-turnend" "$child_wt/.fm-kimi-turnend" "$child_wt/.fm-copilot-turnend"
      fi
    ...
    rm -f "$sub_state/$child_id.status" "$sub_state/$child_id.turn-ended" \
      ...
      "$sub_state/$child_id.copilot-turnend-token"
  done
}
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  if [ -d "$WT" ]; then
    ...
    rm -f "$WT/.claude/settings.local.json" "$WT/.opencode/plugins/fm-turn-end.js" \
      ...
      "$WT/.fm-grok-turnend" "$WT/.fm-kimi-turnend" "$WT/.fm-copilot-turnend"
  fi
...
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.meta" \
  ...
  "$STATE/$ID.kimi-turnend-token" "$STATE/$ID.copilot-turnend-token"
