...
family_for_basename() {
  case "$1" in
    ...
    fm-operational-input.test.sh|fm-pi-primary-types.test.sh|fm-promote.test.sh|\
    ...
    fm-session-lock.test.sh|fm-session-start.test.sh|fm-sessionstart-nudge.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-gate-refuse*|bin/fm-lock*|bin/fm-env-lib.sh)
      printf '%s\n' session-bootstrap
      ;;
