...
# copilot's working footer changed across the supported version range: 1.0.72
# renders "◉ Working esc interrupt" (matched by the shared esc (to )?interrupt
# shape) while 1.0.68 renders "esc cancel", so the copilot signature carries
# BOTH for as long as the spawn version gate floor stays at 1.0.68
# (bin/fm-harness-policy.sh; fixtures in tests/fm-copilot-pane-fixture.test.sh).
...
FM_TMUX_COPILOT_BUSY_REGEX_DEFAULT='esc (to )?interrupt|esc cancel'
