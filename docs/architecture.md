...
The existing turn-end guard remains the final backstop for the five harness-engine protocols that have a tracked hook integration, with pi-signed sharing Pi's protocol and the `--claude` mode cooperating with the auto-arm claim.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux, Herdr, and cmux have verified classifiers for secondmate recovery, Zellij remains unverified, and Orca does not support secondmate spawns.
