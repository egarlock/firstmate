...
cmux's container shape is configurable per home (`config/cmux-container`): one workspace per task with one surface by default, or one tab per task inside firstmate's own (or a shared per-home) container workspace; scoped titles carry the active home label plus a short hash of the resolved `FM_ROOT` path on the task workspace or task tab, and `--secondmate` spawns are refused, mirroring Orca.
