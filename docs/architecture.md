...
Injection requires an affirmatively `empty` composer verdict from the shared classifier (`bin/fm-composer-lib.sh`), so a `pending` composer (real unsubmitted text) and an `unknown` one (an unreadable pane, or a bare shell prompt left after the pane's agent exited) both defer rather than risk typing an escalation into a live shell.
...
Teardown is fail-closed for ship worktrees: if git cannot even resolve `HEAD` in the worktree (a broken `.git` gitfile pointer, a corrupted repo, or a plain non-git directory), teardown refuses up front rather than letting empty git output read as clean-and-published; otherwise the landed-work oracle is a dirty gate followed by three allow-conditions checked in order.
A dirty worktree (uncommitted changes) always refuses; then teardown allows on any one of a recorded `landed=<sha>` in `state/<id>.meta` that covers HEAD (HEAD is an ancestor of the recorded sha, so the merge already happened and nothing was committed past it; recorded by `bin/fm-pr-merge.sh` / `bin/fm-merge-local.sh`), `HEAD` reachable from any publishing remote-tracking branch (a fork counts), or the branch's content already present in the freshly fetched default branch; it refuses only when none of the three holds.
...
The verdict vouches only for the commits it covers, so a `landed=` that no longer covers HEAD (commits made after the merge) or that cannot be resolved (the `pr-<n>` placeholder) falls through to the reachability and content checks instead of allowing, and post-merge commits are never silently discarded.
...
Genuinely unlanded work (no recorded `landed=` covering HEAD, not reachable from any publishing remote, and content not in the default branch) and dirty worktrees still refuse; an inconclusive content check refuses rather than silently allowing.
