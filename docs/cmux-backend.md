...
| Create task workspace | `cmux new-workspace --name <scoped-title> --cwd <dir> --focus true --id-format uuids` | Creates a workspace with exactly one default surface, then restores the prior full focus context to avoid the 0.64.18+ unrealized-surface black-screen defect for TUI agents. The caller passes `fm-<id>`, but the adapter creates `fm-<home-label>-<id>`. |
