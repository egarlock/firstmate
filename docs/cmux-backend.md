...
Verified against the real installed app: cmux 0.64.17 (build 97), macOS aarch64; the tab container mode, its focus-at-birth workaround, the passive cwd tiers, and the lazy-terminal findings below were verified against cmux 0.64.20 (build 100), as was the whole secondmate design ("Secondmate support" below, live pass 2026-07-27).
...
You do not need to bring the window forward for routine supervision: from an active firstmate session, `bin/fm-peek.sh <id>` reads a task's surface without focusing it, and `FM_HOME=<this-firstmate-home> bin/fm-send.sh <id> "<text>"` steers it unless `FM_HOME` is already set to the active firstmate home.
A spawn does take focus while the new terminal is created, though (it must, see "Task and secondmate creation requires focus at birth on cmux 0.64.18+" below), and that restoration is only *within* cmux: it puts you back on the exact window/workspace/pane/tab you were on, so if you were already working inside cmux the cost is a short flicker rather than a lasting steal.
It cannot hand macOS app-level focus back, so if another application was frontmost, the spawn raises cmux and leaves it in front - budget for that when spawning while working elsewhere.
...
Limitations: cmux is experimental, macOS-only, GUI-first (never viable for a headless/CI/SSH-only firstmate instance), and has no native busy-state signal - see "Known gaps left for a follow-up" at the end of this document.
`--secondmate` spawns are supported through the dedicated-workspace-per-mate design in "Secondmate support" below.
...
The tab-mode shared container title is also what a cmux secondmate's dedicated workspace is initially titled for its OWN home, so the mate's workspace doubles as that home's tab-mode container (see "Secondmate support" below).
...
## Task and secondmate creation requires focus at birth on cmux 0.64.18+
...
Any creation path that immediately hosts a full-screen TUI agent now follows one rule: create focused, then restore the prior focus context.
That rule now covers task workspace mode (`new-workspace --focus true`), task tab mode (`new-surface --focus true`), and secondmate workspace create (`new-workspace --focus true`).
Provenance differs across the three: tab mode and secondmate create are device-verified as written (the secondmate correction came from a real launch whose copilot CLI pane stayed black forever), while task workspace mode applies the same pattern by parity with those two and has not been device-verified in this home - its live painted-versus-black confirmation is still outstanding.
...
It is cmux-internal only: `focus-window`/`select-workspace`/`focus-pane`/`reorder-surface` cannot return macOS app-level focus to another application, so a spawn started while a different app was frontmost leaves cmux in front.
...
| Create task workspace | `cmux new-workspace --name <scoped-title> --cwd <dir> --focus true --id-format uuids` | Creates a workspace with exactly one default surface (verified). The `--focus true` plus full focus restore is applied by parity with the device-verified tab-mode and secondmate paths, to avoid the 0.64.18+ unrealized-surface black-screen defect for TUI agents, and is not itself device-verified in this home - see "Task and secondmate creation requires focus at birth on cmux 0.64.18+". The caller passes `fm-<id>`, but the adapter creates `fm-<home-label>-<id>`. |
...
Re-verified 2026-07-27 against the CURRENTLY INSTALLED version's shipped source (cmux 0.64.20 build 100, `cmux version` -> `cmux 0.64.20 (100) [14e3400b9]`; `github.com/manaflow-ai/cmux` commit `14e3400b95` "Bump version to 0.64.20", cloned read-only as scratch; the captain's live app was NOT quit, relaunched, or reconfigured for any of this):

- `Sources/Workspace.swift`'s only `init` still unconditionally sets `self.id = UUID()`, and `id` is declared `let id: UUID` - structurally impossible to reassign after construction, so no restore path can bring an old id back.
- NEW since the 2026-07-04 read: workspaces now also carry `private(set) var stableId = UUID()` ("Restart-stable workspace identifier persisted for durable deep links"), which `restoreSessionSnapshot` DOES adopt from a persisted snapshot.
- But the control socket does not expose it: `Sources/TerminalController+ControlWorkspaceContext.swift` builds each `workspace list`/`workspace.current` row from `workspace.id` - the fresh runtime UUID - so the ids firstmate records still do NOT survive a relaunch. Verdict unchanged; ids cannot be the sole durable handle.
- Load-bearing for the secondmate design below, from the same restore path (`Workspace.swift`, `restoreSessionSnapshot`): PTYs die with the app ("all of which spawn fresh shells - PTYs do not survive an app restart", so a restored mate is always agent-dead), panel/surface ids are re-minted (`oldToNewPanelIds`), while the workspace's custom title (`setCustomTitle(snapshot.customTitle, ...)`) and each tab's custom title (`setPanelCustomTitle(...)`) ARE restored - which is exactly why recovery by last-synced workspace title and by the scoped tab title both remain viable after a relaunch.
...
`--secondmate` spawns are supported on cmux through a dedicated-workspace-per-mate design (captain-resolved 2026-07-27), lifting the refusal this document previously recorded as a known gap.

**Shape.**
A cmux secondmate gets ONE dedicated workspace of its own, created at the mate's home directory with the mate's tab inside it, in EVERY task-container mode - a tab-mode primary must never land a mate as a tab inside the PRIMARY's own workspace (`fm_backend_cmux_create_secondmate`; fm-spawn.sh's cmux arm).
The workspace's initial title is the mate home's own `fm-<home-label>` (e.g. `fm-2ndmate-<id>-<8hex>`), derived with both `FM_HOME` and `FM_ROOT` pointed at the mate home so it is byte-identical to what the mate's own crew spawns later derive - the mate's workspace therefore doubles as its home's tab-mode shared container, and tab-mode crew tabs the mate spawns land in the mate's own workspace.
Workspace-mode crew tasks the mate spawns get their own `fm-2ndmate-<id>-<8hex>-<task>` workspaces; in both modes nothing a mate spawns can land in the primary's workspace, because the home label (with its `FM_ROOT` path hash) differs and tab-mode's ambient `CMUX_WORKSPACE_ID` inside the mate's terminal names the mate's own workspace.
The mate's single tab is renamed to the PRIMARY's scoped task title (`fm-<primary-home-label>-<id>`, sticky per the verified rename-tab finding), which is what `fm_backend_cmux_secondmate_resolve`'s scoped-tab rung recovers the mate by after a relaunch; the rename is fatal on failure, exactly like the tab-mode create's.

**Identity: id-primary, synced-title recovery.**
The captain may retitle the mate's workspace freely - free-form titles apply to the secondmate workspace only, while ordinary task workspaces keep their generated scoped titles.
The recorded workspace/surface UUIDs (`cmux_workspace_id=`/`cmux_surface_id=`, mirrored in `window=`) are the operational handle while the app is alive, and the meta additionally records `cmux_workspace_title=`, the last-synced workspace title.
`fm_backend_cmux_secondmate_resolve` is the supervision touch that keeps that record honest: the session-start secondmate liveness sweep runs it before every probe (`fm_backend_secondmate_resolve` in bin/fm-backend.sh), re-reading the title by id and syncing a captain retitle into the meta.
Every label-gated operation goes through that same resolver too: `fm_backend_cmux_target_ready` detects a `kind=secondmate` meta behind the expected label (`fm_backend_cmux_secondmate_meta_for_label`) and delegates identity to the resolver instead of the scoped-title gate, adopting ONLY the resolved endpoint - so `fm-send`/`fm-peek`/`fm-crew-state`'s capture path and the `agent_alive` probe all keep working on a renamed mate, while ordinary task targets keep the scoped-title gate unchanged.
This closes a live-found bug (2026-07-28): a captain-renamed mate workspace failed the scoped-title gate for every label-gated op even though resolve returned the correct endpoint, because the agent's own terminal-title updates had overwritten the scoped tab title as well, so both title rungs missed.
After a relaunch (socket-reported ids re-minted; see the re-verification above), recovery is strictly record-driven, one rung at a time, each requiring EXACTLY one candidate: the recorded title across every window (`fm_backend_cmux_workspaces_all_windows` - plain `workspace list` is current-window-scoped), then the scoped tab title (tab custom titles survive restore), then the fingerprint of last resort - a workspace whose creation cwd is the mate's home AND holding a surface whose passive tty-derived cwd (tty via `cmux tree`, foreground pid via `ps`, cwd via `lsof` - never the active pwd probe, which would type into unproven terminals) is that home.
Multiple candidates at any rung refuse loudly with the candidates on stderr (rc 3) - never guess, never claim another home's workspace; no candidate at all is the definitively-gone verdict (rc 2), which the sweep treats as the confident dead reading.
Teardown and the sweep close a mate's WHOLE workspace only through that same resolver (`fm_backend_cmux_secondmate_kill`), so cleanup of one mate can never touch another home's workspace or the primary's.

**Agent liveness (`fm_backend_cmux_agent_alive`).**
A real agent-process check, not pane presence: the mate surface's tty (from `cmux tree`) is classified by EVERY process on it - any verified-harness name is `alive`; nothing but shells and the resident `/usr/bin/login` wrapper cmux parents every tab shell through is `dead`; anything else (pi's bare `node`, an editor) is `unknown` and never acted on.
Classifying every tty process rather than the last foreground entry is deliberate: a live agent's tool child (e.g. a bash it shelled out to) can transiently be the foreground process and must not read as a dead shell.
A `down` socket is confidently `dead` (cmux hosts every task pty, so no cmux-backed agent can outlive the app); `denied`/`unauth` sockets and never-started (tty-less) terminals are `unknown`.

**Backend selection (`config/secondmate-backend`).**
An explicit `--backend` always wins; a respawn reuses the backend recorded in the mate's meta; otherwise the first non-empty line of local gitignored `config/secondmate-backend` applies when not `default`, then the shared resolution chain - [`docs/configuration.md`](configuration.md) "Runtime backend" owns the schema and bootstrap validation.

**Verified live 2026-07-27** against the installed cmux 0.64.20 (build 100), macOS aarch64, socket in automation mode; the captain's app was never quit, relaunched, or reconfigured, only `fm-test-`-marked scratch artifacts were created, and every one was confirmed removed afterward (a full-app title scan for `test` matched nothing):

1. `tests/fm-backend-cmux-smoke.test.sh`'s secondmate section, all against the real app with a scratch `test-`-marked home: `create_secondmate` (dedicated workspace at the mate home, scoped tab title), resolve-by-live-id, a REAL captain retitle via `cmux workspace rename <ws> --title <new>` synced into the meta on the next resolve, stale-id recovery by the synced title with ids re-recorded, `agent_alive` reading a woken bare-shell tab as confidently `dead`, fingerprint recovery after BOTH titles were deliberately lost (real `ps`/`lsof` passive tiers), and the meta-driven kill closing the whole workspace idempotently.
2. A full live lifecycle through this branch's own scripts, scratch `FM_HOME` and scratch mate home, real `claude` harness: `bin/fm-spawn.sh test-smlive98220 <home> --secondmate` with `config/secondmate-backend` = `cmux` (no `--backend` flag) spawned a real claude mate, printing `window=<ws-uuid>:<sf-uuid>` and recording `backend=cmux`, both ids, `cmux_workspace_title=fm-2ndmate-test-smlive98220-059370cf`, and `home=` in the meta.
3. `fm_backend_cmux_agent_alive` read `alive` against the running claude; two `C-c` sends left it `alive` (interrupt only - a tool-child/foreground blip never misreads as dead), and `/exit` flipped it to `dead` (bare shell behind the login wrapper).
4. A real `bin/fm-bootstrap.sh` run against that scratch home then exercised the sweep's whole loop silently (no `SECONDMATE_LIVENESS:` line): the dead mate's workspace was closed and a fresh one spawned, with the meta re-recording the NEW workspace/surface uuids under the same synced title, and the respawned claude probing `alive`.
5. `fm_backend_cmux_secondmate_kill` on the meta closed the respawned mate's whole workspace after `/exit`; the residue scan then matched zero `fm-test`/`fm-2ndmate-test` titles anywhere in the app.

One caveat observed during the first live pass: test workspaces created into the captain's then-active second macOS window all died when that window was closed by hand mid-run - a window close takes its workspaces' ptys with it, exactly like an app relaunch but scoped to one window.
For a real mate that shape is recovered by the same ladder (gone endpoint -> confident dead -> respawn), and the test-safety guards now do their title confirmation across every window (`tests/cmux-test-safety.sh`), not just the current one, so cleanup cannot be defeated by a focus change.

**Verified live 2026-07-28** against the installed cmux 0.64.20 (build 100), macOS aarch64: the renamed-workspace label-gate fix, exercised read-only against the fleet's two live renamed mates (workspace titles `Mate - Dashboard` and `Mate - Copilot Setup`, both with agent-overwritten tab titles), using scratch COPIES of their metas so no primary-home record was touched, and with no send, retitle, or close performed against either mate:

1. Unpatched (`d17bf3c`), `fm_backend_cmux_target_ready "<recorded window>" fm-dash-mate` returned rc 1 and `fm_backend_cmux_agent_alive` printed `unknown` - the reproduction of the live failure (`fm-send`'s "cmux send failed", `fm-peek`'s empty output, `fm-crew-state`'s "backend target gone", and the sweep's "liveness probe inconclusive" all funnel through that same gate).
2. Patched, for BOTH mates: `target_ready` succeeded echoing exactly the recorded `<workspace_uuid>:<surface_uuid>`, `agent_alive` printed `alive` (real claude on the surface tty), and a bounded `fm_backend_cmux_capture` returned the mates' real pane content (claude's idle footer).
3. The full real-app smoke suite (`tests/fm-backend-cmux-smoke.test.sh`, `fm-test-`-prefixed artifacts only, guarded cleanup) passed end to end on the patched adapter, confirming ordinary-task flows unregressed live.
...
- **A secondmate whose workspace title was BOTH retitled-and-never-synced and whose tab title was lost can only be recovered by the home-cwd fingerprint**, which requires a started terminal (a tty) parked at the mate's home; a relaunch-restored mate whose terminal never started AND whose titles were all lost is reported gone rather than recovered - acceptable, because the sweep's respawn recreates it deterministically ("Secondmate support" above owns the ladder and its refusal rules).
