...
The mate's single tab is renamed to the PRIMARY's scoped task title (`fm-<primary-home-label>-<id>`, sticky per the verified rename-tab finding), which is what `fm_backend_cmux_target_ready`'s tab arm already recovers ordinary send/peek/watch operations by; the rename is fatal on failure, exactly like the tab-mode create's.
