...
FM_TEARDOWN_NET_TIMEOUT=30    # seconds allowed for the content-fallback git fetch in fm-teardown.sh's landed-work check; on timeout the fetch fails, the content check is inconclusive, and teardown refuses
...
FM_PROC_ROOT_OVERRIDE=  # alternate /proc root for fm_pid_identity; mainly for tests, so the Linux /proc identity branch stays exercisable on any host
