...
`kimi` has no tracked session-start integration.
Kimi Code exposes only global `[[hooks]]` configuration in `~/.kimi-code/config.toml`, so the tracked-transport pattern every row above relies on cannot be used.
...
It also verifies that every verified adapter either registers a tracked transport or is explicitly recorded above as having no tracked session-start integration.
