...
# fm-operational-input.sh is the owner that the extracted fm-marker-lib.sh
# compatibility shim sources unconditionally at load time (#909); it is a
# copied sibling for the same reason - the old fm-send.sh aborts under set -eu
# while sourcing fm-marker-lib.sh if it is missing from the old bin/.
OLD_BIN_UNCHANGED_SIBLINGS="fm-gate-refuse-lib.sh fm-guard.sh fm-lock-lib.sh fm-harness-policy.sh fm-tasks-axi-lib.sh fm-pr-lib.sh fm-tangle-lib.sh fm-tmux-lib.sh fm-composer-lib.sh fm-wake-lib.sh fm-classify-lib.sh fm-supervision-lib.sh fm-ff-lib.sh fm-config-inherit-lib.sh fm-project-mode.sh fm-harness.sh fm-crew-state.sh fm-decision-hold.sh fm-backend.sh fm-operational-input.sh"
