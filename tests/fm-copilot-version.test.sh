...
# The copilot gate must resolve through the shared policy probe, and fm-spawn
# must not grow a private dotted-triple parser beside it. (Folding bootstrap's
# and tasks-axi-lib's own version probes onto the shared parser is the tabled
# fork-side unification; it is deliberately NOT asserted on the refork
# candidate - see data/refork-plan-v2.md captain decisions.)
test_version_probe_has_one_owner() {
  grep -q 'fm_copilot_compatible' "$ROOT/bin/fm-spawn.sh" \
    || fail "bin/fm-spawn.sh no longer gates copilot launches through fm_copilot_compatible"
  grep -q 'fm_harness_version_parts' "$ROOT/bin/fm-spawn.sh" \
    || fail "bin/fm-spawn.sh no longer reports the found version through fm_harness_version_parts"
  grep -qE '^[a-z_]*version_parts\(\)' "$ROOT/bin/fm-spawn.sh" \
    && fail "bin/fm-spawn.sh re-declared a private version parser instead of using the shared one"
  pass "the copilot version gate resolves through the shared policy probe"
}
...
test_spawn_aborts_on_incompatible_copilot() {
  ...
  out=$(FM_ROOT_OVERRIDE='' FM_HOME="$home" \
    ...
    PATH="$fb:$PATH" "$SPAWN" "$id" "$proj" copilot --mode no-mistakes --yolo off 2>&1)
  status=$?
...
test_spawn_skip_env_bypasses_gate() {
  ...
  out=$(FM_ROOT_OVERRIDE='' FM_HOME="$home" \
    ...
    PATH="$fb:$PATH" "$SPAWN" "$id" "$proj" copilot --mode no-mistakes --yolo off 2>&1) || true
