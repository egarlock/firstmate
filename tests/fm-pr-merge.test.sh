...
#       propagates az failures, and records only pr=/pr_head=, never landed=
set -u
...
test_ado_merge_completes_and_records() {
  ...
  # The merge path records only pr= and pr_head=; pin that no landed= verdict
  # is written here.
  assert_no_grep 'landed=' "$case_dir/state/task-x1.meta" \
    "ado-completes: the merge path must not record a landed= verdict"
