...
run_spawn() {
  ...
  # empty instead of leaking the invoking shell's value, which would make the
  # launch assertions depend on the developer's environment.
...
# provider axis is orthogonal to it, so these tests pass a fixed valid one.
run_ship_spawn() {
...
test_valid_provider_prefixes_env_and_records_meta() {
  ...
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --provider kimi)
  status=$?
...
test_unknown_provider_refused() {
  ...
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --provider nosuch)
  status=$?
...
test_bad_provider_name_refused() {
  ...
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --provider 'Bad_Name')
  status=$?
...
test_malformed_provider_lines_refused() {
  ...
  do
    ...
    out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --provider bad)
    status=$?
...
test_provider_with_non_claude_harness_refused() {
  ...
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --harness codex --provider kimi)
  status=$?
...
test_provider_with_raw_launch_refused() {
  ...
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" 'claude --custom-flag' --provider kimi)
  status=$?
...
test_effort_flag_omitted_with_provider() {
  ...
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --provider kimi --effort high)
  status=$?
