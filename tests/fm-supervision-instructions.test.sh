...
# Deliberate exemptions: pi-signed shares Pi's engine and renders pi.md under
# its own heading (asserted by its own test below), and kimi as a primary is
# unsupported upstream, so its unknown fallback is intentional and asserted
# here instead of exempted silently.
    ...
    case "$harness" in
      ...
      kimi)
        ...
        assert_contains "$out" "Mode: Unknown harness fallback." \
          "kimi has no primary protocol and must render the unknown fallback deliberately"
        continue
        ;;
    esac
